...
export function OperatorPanel({ input, source, onSource, settings, onSettings, cursor, onCursor, elementProgress, onElementProgress, preview, onPreview, landmarks, onLandmarks, probabilities, onProbabilities, fps, onFps, onCalibrate, onClearCalibration, children }: {
  ...
  return <>
    ...
    <label>Smoothing <select value={settings.smoothing.method} onChange={e => onSettings({ ...settings, smoothing: e.target.value === 'ema' ? { method: 'ema', alpha: 0.15 } : { method: 'kalman', processNoise: 10000, measurementNoise: 100 } })}><option value="ema">EMA</option><option value="kalman">Kalman</option></select></label>
