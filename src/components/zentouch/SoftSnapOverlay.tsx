...
export function SoftSnapOverlay({ snapshot, targets, timing, cursor = defaultCursorSettings }: SoftSnapOverlayProps) {
  ...
  if (!glow) return null
  return (
    <div className="soft-snap" aria-hidden="true" data-phase={glow.phase} data-style={cursor.style} data-family={cursorStyles.find(style => style.id === cursor.style)?.family}
