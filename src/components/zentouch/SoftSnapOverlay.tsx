...
import type { EngineSnapshot } from '../../types/interaction'
...
export function SoftSnapOverlay({ snapshot, timing, cursor = defaultCursorSettings }: SoftSnapOverlayProps) {
  const glow = softSnapModel(snapshot, timing)
  ...
  useLayoutEffect(() => {
    ...
    // The camera pipeline already applies EMA. Render its position
    // directly, without a second motion filter or a movement dead zone.
