...
Score eligible targets using the SPEC's alignment, distance, motion, history, and UI-prior weights, then apply softmax for relative ranking and multiply each target probability by absolute proximity evidence. The live model uses `exp(-(edgeDistance / 48)^2)`, with edge distance in CSS pixels and zero inside the target rectangle. Rejected mass belongs to an implicit no-target outcome. Disabled targets must be excluded before softmax: setting their prior to zero alone would still give them positive probability. Omitted priority means 1; priority zero removes only the prior contribution, not eligibility.

`probability` is the proximity-weighted softmax result; `belief` is its temporal smoothing using `temporalAlpha`. Each target distribution sums to at most 1; its residual is the probability/belief of no target. Do not renormalize target mass to 1. On registry changes, remove ineligible entries and retain surviving historical belief at its absolute value before blending; removed mass transfers to no target. New targets start with zero historical mass, or current probabilities when there was no previous target distribution. Empty eligibility or unusable tracking produces an empty distribution and null leader. Target IDs occur once per distribution.

Return a null leader when no-target belief is at least as large as the greatest target belief. Otherwise choose the greatest target belief as leader, retain the previous leader on an exact tie if possible, otherwise use lexicographic target ID order. Event/state confidence is the target's temporal belief, not landmark confidence. Tracking confidence independently gates interaction. Soft-snap rendering uses pointing position, leader geometry, belief, and progress; it does not replace intent scoring with nearest-target hit testing.
