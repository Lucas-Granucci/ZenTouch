...
Small-target assistance uses the shorter registered dimension in CSS pixels: none at 96 px or above, increasing linearly to full assistance at 32 px or below. It lowers each confidence threshold by up to the smaller of 0.15 and 25% of that threshold (default lock 0.85 → 0.70; highlight 0.60 → 0.45). Both current probability and temporal belief must exceed the adjusted thresholds. Containment, proximity limits, tracking requirements, and lock/dwell durations still apply. The visual cursor multiplies configured snap strength by up to two, capped at 1 before multiplying by belief; setting snap strength to zero disables attraction. Geometry updates immediately recalculate assistance. These are initial tuning values, not benchmark-derived defaults.
